"""Ember — single-device reference implementation.

The spec. Read this to understand the optimizer; the distributed version (`ember.py`)
produces bit-identical updates, just placed correctly across ranks.

Ember optimizes an embedding table E in R^{V x D} with a V+D *factored* second moment
and NO first moment — so optimizer state is O(V + D) instead of Adam's O(2 V D)
(~1500x smaller at a 50K vocab). It matches AdamW quality on embedding tables.

Update, for embedding gradient G in R^{V x D} (sparse over V — only rows of tokens in
the batch get a gradient):
...
"Sparse-aware" col: average g^2 only over rows that actually got a gradient this step,
else inactive rows would dilute the column statistic toward zero.
"""
import torch


class EmberReference(torch.optim.Optimizer):
